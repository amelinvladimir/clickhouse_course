...
### Смотрим содержимое ClickHouse Keeper
```sql
SELECT 
	*
FROM system.zookeeper
...
### Вставляем строку, предварительно выключив первую ноду
```sql
INSERT INTO learn_db.mart_student_lesson 
SELECT
	floor(randUniform(2, 10000000)) as student_profile_id,
...
### Создаем таблицу для формирования набора строк, которые затем будем несколько раз вставлять в основную таблицу
```sql
...
### Наполняем временную таблицу строками
```sql
...
### Несколько раз выполняем вставку строк из временной таблицы в основную и проверяем, меняется ли количество строк в основной таблице
```sql
INSERT INTO learn_db.mart_student_lesson
...
### Создаем таблицу с движком MergeTree и несколько раз вставляем в нее один и тот же набор строк. Убеждаемся, что строки вставляются несколько раз
```sql
