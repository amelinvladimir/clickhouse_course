...
### Создаем реплицируемые таблицы отдельно на каждой ноде
```sql
CREATE TABLE learn_db.mart_student_lesson 
(
	`student_profile_id` Int32, -- Идентификатор профиля обучающегося
...
### Проверяем, что в таблице появилась строка после вставки на первой ноде
```sql
SELECT * FROM learn_db.mart_student_lesson;
```

### Вставляем данные на второй ноде 
```sql
INSERT INTO learn_db.mart_student_lesson 
SELECT
	floor(randUniform(2, 10000000)) as student_profile_id,
...
### Проверяем, что новая строка появилась на второй ноде
```sql
SELECT * FROM learn_db.mart_student_lesson;
```

### Меняем person_id_int у оставшейся строки и проверяем, что id 
```sql
ALTER TABLE learn_db.mart_student_lesson UPDATE person_id_int = 1 WHERE person_id_int > 1;
```

### Проверяем, что person_id_int изменен
```sql
SELECT * FROM learn_db.mart_student_lesson;
```

### Вставляем 10 000 000 строк
```sql
INSERT INTO learn_db.mart_student_lesson 
SELECT
	floor(randUniform(2, 10000000)) as student_profile_id,
...
### Смотрим, из какиз частей данных состоит таблицу и историю изменения частей данных
```sql
select * from system.parts where table = 'mart_student_lesson' order by part_type;
select * from system.part_log order by event_time desc;
```
